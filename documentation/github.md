...
>[!IMPORTANT]
>*Always use `dev` as the base for your branch when you create a new branch.*
>
> *`main` is always the latest stable build that is deployed on the server*

### Git Bash
If you're using git bash for your version control method follow these steps to create a new branch.
1. Start by checking out the `dev` branch with the `git checkout dev`.
2. Create a new branch with `git checkout -b <name of your branch>`. Now your branch is created locally and needs to be published to GitHub.
...
2. Create a new branch by going to `... > Branch > Create Branch From ...` and selecting `origin/dev`.
...
When creating a new branch it is important to make sure that you connect it with a specific issue. The naming convention should be the following:
`discipline`-`addition type`-`issue ID`
...
| backend   | This branch is dedicated to backend development            |
| devops    | This branch is dedicated to devops development            |
| dev       | This branch is dedicated to cross-discipline development *(two or more discipline need to work at the same time)*     |

| ADDITION TYPE  |  DESCRIPTION                                                   |
| :--            | :--                                                            |
| feature        | This is a new feature that has yet to be added                 |
| fix            | This is a fix of a bug/issue from an already implemented feature |
...
- ***Issue ID*** - Specify the issue *(feature/fix)* that this pull request aims to solve. To specify the issue ID write `closes #<issue-id>` in the description of the pull request. This will automatically close the issue when the pull request is approved.
- ***Title*** - The title of the pull request, a quick summery of what the pull request is about
- ***Description*** - Specify what the pull request accomplishes in more detail and potential things to take note of that might be important for other developers.
- *** Assignees *** - Who is involved with the pull request.
- ***Reviewer*** - Who should review this other but yourself? 
  - *Recommendation: Ask someone who works in the same discipline as you.*
- ***Labels*** -  It is recommended to put labels on your pull request to make sure that it is obvious what discipline this belongs to.
